...
function changePrixAchat(id) {
	...
	link = "modif-prixachat.php?id=" + id + "&prix=" + prix + "&rand=<? echo rand(0,1000000) ?>";
	//alert(link);
										...
										<tr>
										   ...
										   <tbody>
											  ...
											  ?>
												<tr>
													<td class="highlight">
														<div class="success"></div> <a href="produit.php?modif_num=<? echo crypte($rcc[$nom_champ . "_num"]) ?>"> <? echo $rcc["produit_nom"] ?></a> (<? echo $nbr ?>)</td>
													<td><? echo $rcc["categorie_nom"] ?></td>
													<td><? echo $rcc["marque_nom"] ?></td>
													<td><input type="text" name="prixachat" id="prixachat_<? echo $rcc["prixachat_num"] ?>" value="<? echo $prix_achat ?>" onBlur="changePrixAchat(<? echo $rcc["prixachat_num"] ?>)" class="form-control"></td>
													<td><? 
													if ($prix!=0)
													...
													<td><? if ($rcc["produit_etat"]==1) 
															echo "Visible";
														   else
															echo "Invisible"; ?>
													</td>
													<td>
														<a href="produit.php?modif_num=<? echo crypte($rcc["produit_num"]) ?>" class="btn btn-outline btn-circle btn-sm purple">
															<i class="fa fa-edit"></i> Edit </a> 
														<a href="<? echo $_SERVER["PHP_SELF"] . '?suppr=' . crypte($rcc["produit_num"]) ?>" class="btn btn-outline btn-circle dark btn-sm black" onClick="return confirme()">
