...
switch ($data["event"]) {
    case "invitee.created" : // Nouvelle inscription
        ...
        $nbr = count($rtt);
        if ($nbr==0) {
            $sql = "insert into clients values (0,0," . sql_safe($user["nom"]) .  "," . sql_safe($user["prenom"]) .  ",'',''," . sql_safe($urser["cp"]) .  ",''," . sql_safe($user["tel"]) .  "," . sql_safe($user["email"]) .  "," . sql_safe($user["date-mariage"]) .  ",'','','','3'," . sql_safe($user_num) .  ",'" . Date("Y-m-d H:i:s") .  ",'" . Date("Y-m-d H:i:s") .  ",'','','','','','','','','','','','',0,0)";
