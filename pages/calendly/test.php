...
switch ($data["event"]) {
    case "invitee.created" : // Nouvelle inscription
        ...
        $nbr = count($rtt);
        if ($nbr==0) {
            $sql = "insert into clients values (0,0," . safe_sql($user["nom"]) .  "," . safe_sql($user["prenom"]) .  ",'',''," . safe_sql($urser["cp"]) .  ",''," . safe_sql($user["tel"]) .  "," . safe_sql($user["email"]) .  "," . safe_sql($user["date-mariage"]) .  ",'','','','3'," . safe_sql($user_num) .  ",'" . Date("Y-m-d H:i:s") .  ",'" . Date("Y-m-d H:i:s") .  ",'','','','','','','','','','','','',0,0)";
