...
// ———————————————————————————————————
/** Affiche le devis en HTML comme avant, mais renvoyé dans JSON */
function api_afficheDevis(int $id): string {
    /** @var Db $base */ global $base;
    /** @var User $u */  global $u;

    // Récup header devis + paiement
    ...
    $commande = montantCommande((int)$rcc["id"]); // suppose helper existant

    // Liste produits
    $sql = "SELECT cp.*, p.*, t.*, m.*, c.*
    ...
    return html_out(function() use ($pp, $base, $rcc, $commande, $id, $u) {
        foreach ($pp as $rpp) {
            ...
            $rss  = $base->queryRow($sqlS);
            $stock = $rss ? (int)$rss["stock_virtuel"] : 10;

            echo '<tr>';
            echo '<td><img src="'.h($image_pdt["min"] ?? '').'" style="width:90px"/></td>';
            echo '<td>'.h($rpp["categorie_nom"]).'<br>'.h($rpp["marque_nom"]).'<br><strong>'.h($rpp["produit_nom"]).'</strong></td>';

            // Taille
            $selectId = 'taille_'.$rpp["produit_num"].'_'.$rpp["taille_num"];
            echo '<td><select name="'.h($selectId).'" id="'.h($selectId).'" onChange="modifTaille('.(int)$id.','.(int)$rpp["produit_num"].','.(int)$rpp["taille_num"].');">';
            echo '<option value="-1">A renseigner</option>';
            $sqlT = "SELECT t.* FROM tailles t
            ...
            foreach ($ss as $st) {
                ...
                echo '<option value="'.(int)$st["taille_num"].'"'.$sel.'>'.h($st["taille_nom"]).'</option>';
            }
            echo '</select></td>';

            echo '<td>'.safe_number_format($rpp["montant_ttc"],2,"."," ").' €</td>';

            // Qte
            $qSelId = 'qte_'.$rpp["produit_num"].'_'.$rpp["taille_num"];
            echo '<td align="center"><select name="'.h($qSelId).'" id="'.h($qSelId).'" onChange="modifQte('.(int)$id.','.(int)$rpp["produit_num"].','.(int)$rpp["taille_num"].');">';
            for ($i=0;$i<=$stock;$i++) {
            ...
            // Prix total
            echo '<td>';
            if ((float)safe_number_format($prix_total_ttc,2) <= 0.0) echo 'OFFERT';
            else echo safe_number_format($prix_total_ttc,2,"."," ").' €';
            echo '</td>';
        ...
        // Totaux
        echo '<tr><td colspan="5" align="right"><strong>Total HT</strong></td><td colspan="2">'.safe_number_format($commande["commande_ht"],2,"."," ").' €</td></tr>';
        echo '<tr><td colspan="5" align="right"><strong>TVA (20%)</strong></td><td colspan="2">'.safe_number_format($commande["commande_tva"],2,"."," ").' €</td></tr>';
        echo '<tr><td colspan="5" align="right"><strong>Total TTC</strong></td><td colspan="2">'.safe_number_format($commande["commande_ttc"],2,"."," ").' €</td></tr>';
        ...
        echo '<tr><td colspan="5" align="right"><strong>Total à payer</strong></td><td colspan="2">'.h($montant_a_payer).' €</td></tr>';

        // Paiement
        echo '<tr><td colspan="5" align="right"><strong>Méthode de paiement</strong></td><td colspan="2"><select name="paiement_'.(int)$rcc["id"].'" id="paiement_'.(int)$rcc["id"].'" onChange="modifPaiement('.(int)$rcc["id"].')">';
        $pp = $base->query("SELECT * FROM paiements ORDER BY paiement_pos ASC");
        foreach ($pp as $rpp) {
            $sel = ((int)$rpp["paiement_num"] === (int)$rcc["paiement_num"]) ? ' selected' : '';
            echo '<option value="'.(int)$rpp["paiement_num"].'"'.$sel.'>'.h($rpp["paiement_titre"]).'</option>';
        }
        echo '</select></td></tr>';
        ...
        if ((int)$rcc["paiement_nombre"] > 1) {
            ...
            foreach ($echeance as $val) {
                ...
                echo '<tr><td colspan="6" align="right"><strong>Acompte '.$i.' ('.h($val).'%)</strong></td><td>'.$acompte_val.' €</td></tr>';
                $i++;
            }
        }
        ...
        echo '<tr><td colspan="7" align="right">
                <a href="/clients/client?client_num='.h(crypte((string)$rcc["client_num"])).'&tab=tab_1_3" class="btn red">Fermer</a>
                <a href="/clients/client?client_num='.h(crypte((string)$rcc["client_num"])).'&tab=tab_1_4&commande_passage='.h(crypte((string)$rcc["id"])).'" class="btn blue" onClick="return confirme_commande('.(int)$rcc["id"].')">Passer la commande</a>
              </td></tr>';
    });
}

/** Recalcule les totaux d’une commande (version 2023) */
function api_recalculMontantCommande2023(int $id): void {
    /** @var Db $base */ global $base;
    $sql = "SELECT cp.*, p.*, t.*
            FROM commandes_produits cp
            JOIN md_produits p ON cp.produit_num=p.produit_num
...
try {
    switch ($mode) {
        case 1: // add widget
        {
            ...
            $html = html_out(function() use ($pp, $base, $selection) {
                if (count($pp)>0) {
                                ...
                                <div class="mt-card-item">
                                  <div class="mt-card-avatar mt-overlay-1">
                                    ...
                                    <div class="mt-overlay">
                                      <ul class="mt-info">
                                        <li><a class="btn default btn-outline" href="javascript:addWidget('.(int)$selection.','.(int)$rpp["produit_num"].',2)"><i class="fa fa-trash"></i></a></li>
                                      </ul>
                                    </div>
                                  </div>
            ...
            echo json_encode(['ok'=>true,'html'=>$html,'place'=>'select_'.$selection]); break;
        }
        ...
        case 4: // modif qte
        {
            ...
            api_recalculMontantCommande2023($devis);
            $html = api_afficheDevis($devis);
            echo json_encode(['ok'=>true,'html'=>$html,'place'=>'devis_'.$devis]); break;
        }
        ...
        case 9: // commande fournisseur toggle
        {
            ...
            if ($val===1) {
                ...
                echo json_encode(['ok'=>true,'html'=>date("d/m/Y"),'place'=>'fournisseur_date_'.$id]);
            } else {
                $base->query("DELETE FROM commandes_fournisseurs WHERE id='".$id."'");
                echo json_encode(['ok'=>true,'html'=>'','place'=>'fournisseur_date_'.$id]);
            }
            break;
        }
