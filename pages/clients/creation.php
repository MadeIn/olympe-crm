...
if (isset($ajout)) {
	...
	$nbr = count($tt);
	if ($nbr==0) {
		$sql = "insert into clients values (0," . sql_safe($genre) . "," . sql_safe($nom) . "," . sql_safe($prenom) . "," . sql_safe($adr1) . "," . sql_safe($adr2) . "," . sql_safe($cp) . "," . sql_safe($ville) . "," . sql_safe($tel) . "," . sql_safe($mail) . "," . sql_safe($date!=='' ? $date : '0000-00-00') . "," . sql_safe($lieu) . "," . sql_safe($remarques) . "," . sql_safe($connaissance) . "," . sql_safe($showroom) . "," . sql_safe($u->mNum) . "," . sql_safe(Date("Y-m-d H:i:s")) . "," . sql_safe(Date("Y-m-d H:i:s")) . "," . sql_safe((int)$poitrine) . "," . sql_safe((int)$sous_poitrine) . "," . sql_safe((int)$taille) . "," . sql_safe((int)$hanche1) . "," . sql_safe((int)$hanche2) . "," . sql_safe((int)$carrure_avant) . "," . sql_safe((int)$carrure_dos) . "," . sql_safe((int)$longueur_dos) . "," . sql_safe((int)$biceps) . "," . sql_safe((int)$taille_sol) . "," . sql_safe((int)$pointure) . "," . sql_safe((int)$tour_taille) . "," . sql_safe($interet) . ",0)";
		$base->query($sql);
	} else {
