<? include("/home/madeinpr/www/CRM/olympe-mariage/inc/param_auto.php"); 
	...
	// Envoyer en cron tous les matins � 9h
	...
	while ($rcc=mysql_fetch_array($cc)) {
		...
		if ($test==0) {
			...
			if ($nbr_ta==0) {
				...
				$message = "pour le premier essayage avec notre couturi�re";
				...
				if ($rco["paiement_nombre"]>1) { // ON affiche les acomptes
					...
					foreach ($echeance as $val) {
						...
						$acompte_num++;
						if ($acompte_num==2) { // On recupere la somme du deuxi�me acompte
							$acompte2 = $acompte_val;
						}
					}				
					// On construite le message
					$info_paiement = '<ul>
										<li><b>Montant � payer avant notre rendez-vous</b> : ' . $acompte2 . ' &euro;</li>
										<li>R�gler votre acompte par virement :
											<ul>
												<li>IBAN : ' .  $rco["banque_iban"] . '</li>
												<li>Code BIC / SWIFT : ' . $rco["banque_swift"] . '</li>
											</ul>
										</li>
										<li>Retrouvez les d�tails de votre commande en <a href="https://crm.olympe-mariage.com/commandes/index.php?cde=' . crypte($rco["commande_num"]) . '">cliquant ici</a></li>
									</ul';										
				}
			}
